...
function SessionFullTraceStats({ sessionId }) {
  ...
  useEffect(() => {
    console.log(`[SessionFullTrace] Connecting for session: ${sessionId}`);
    ...
    socket.on('connect', () => {
      console.log(`[SessionFullTrace] Connected, joining room: full_trace_${sessionId}`);
      setConnected(true);

      // Use different event name to distinguish from SessionRealtimeStats
      socket.emit('join_full_trace_session', { session_id: sessionId });
    });

    socket.on('disconnect', () => {
      console.log('[SessionFullTrace] Disconnected from WebSocket');
      setConnected(false);
    });

    socket.on('joined_full_trace_session', (data) => {
      console.log('[SessionFullTrace] Joined session room:', data);
    });

    // Listen to different event name
    socket.on('full_trace_stats_update', (data) => {
      if (data.session_id === sessionId) {
        console.log('[SessionFullTrace] Received stats for session:', data.stats);
        setStats(data.stats);
      }
    });
    ...
    return () => {
      console.log(`[SessionFullTrace] Leaving room: full_trace_${sessionId}`);
      socket.emit('leave_full_trace_session', { session_id: sessionId });
      socket.disconnect();
    };
  }, [sessionId]);
    ...
    return (
      <div className="realtime-container">
        <div className="realtime-disabled-notice">
          <h3>⏳ Đang tải session full trace stats...</h3>
    ...
    <div className="realtime-container">
      <div className="realtime-header">
        <div className="realtime-header-left">
          <h2>📊 Session Full Trace Statistics - {sessionId}</h2>
          <div className="connection-status">
            <div className={`status-dot ${connected ? 'connected' : 'disconnected'}`}></div>
...
export default SessionFullTraceStats;
