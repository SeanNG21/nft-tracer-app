...
struct trace_event {
    ...
    u16 rule_seq;
    u64 rule_handle;
    u64 last_rule_handle;  // NEW: Track last seen rule to detect rule changes
...
// Helper: extract rule handle - IMPROVED VERSION
...
int kprobe__nft_do_chain(struct pt_regs *ctx)
{
    ...
    evt.rule_seq = 0;           // IMPROVED: Reset rule counter
    evt.last_rule_handle = 0;   // IMPROVED: Reset last seen handle
...
int kprobe__nft_immediate_eval(struct pt_regs *ctx)
{
    ...
    stored->last_rule_handle = rule_handle;
