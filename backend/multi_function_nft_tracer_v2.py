...
class MultiFunctionNFTTracerV2:
    ...
    def __init__(self, config_file: str = None, max_functions: int = 50, verbose: bool = False, print_interval: int = 100):
        ...
        self.stats = {
            ...
            'non_ip_packets': 0,
            'parse_failed': 0,
            'by_eth_protocol': defaultdict(int),
            'by_ip_version': defaultdict(int),
        }
...
"""
    ...
    def _handle_event(self, cpu, data, size):
        ...
        if evt['is_non_ip']:
            ...
            if evt['eth_protocol']:
                self.stats['by_eth_protocol'][evt['eth_protocol']] += 1
        if evt['parse_failed']:
            self.stats['parse_failed'] += 1
        if evt['ip_version']:
            self.stats['by_ip_version'][evt['ip_version']] += 1
    ...
    def _print_event(self, evt):
        ...
        # Handle non-IP and parse failed packets
        if evt.get('is_non_ip'):
            eth_proto = evt.get('eth_protocol', 'unknown')
            hook = HOOK_NAMES.get(evt['hook'], f"hook_{evt['hook']}")
            verdict_str = ""
            if evt.get('verdict') is not None and evt['verdict'] != 255:
                verdict_str = f" [{VERDICT_NAMES.get(evt['verdict'], '???')}]"
            print(f"[{self.total_events:4d}] {event_type:10s} {layer:10s} {func:30s} "
                  f"[Non-IP: {eth_proto}] Hook: {hook}{verdict_str}")
            return
        elif evt.get('parse_failed'):
            hook = HOOK_NAMES.get(evt['hook'], f"hook_{evt['hook']}")
            print(f"[{self.total_events:4d}] {event_type:10s} {layer:10s} {func:30s} "
                  f"[Parse Failed] Hook: {hook}")
            return

        # Format IP flow
        src_ip = evt['src_ip'] if evt['src_ip'] else '?.?.?.?'
        dst_ip = evt['dst_ip'] if evt['dst_ip'] else '?.?.?.?'
        flow = f"{src_ip}:{evt['src_port']} → {dst_ip}:{evt['dst_port']}"
    ...
    def _print_statistics(self):
        ...
        print(f"Non-IP packets:     {self.stats['non_ip_packets']}")
        print(f"Parse failed:       {self.stats['parse_failed']}")
        ...
        if self.stats['by_ip_version']:
            print("\nPackets by IP Version:")
            for version, count in sorted(self.stats['by_ip_version'].items()):
                print(f"  IPv{version}:          {count:6d}")

        if self.stats['by_eth_protocol']:
            print("\nNon-IP Packets by Ethernet Protocol:")
            # Map common Ethernet protocols
            eth_proto_names = {
                '0x806': 'ARP',
                '0x8100': 'VLAN',
                '0x86dd': 'IPv6',
            ...
            for proto, count in sorted(self.stats['by_eth_protocol'].items()):
                proto_name = eth_proto_names.get(proto, proto)
                print(f"  {proto_name:15s}: {count:6d}")
    ...
    @staticmethod
    def _format_ip(ip):
        if ip == 0 or ip is None:
            return None  # Instead of "0.0.0.0"
        try:
