#!/usr/bin/env python3
"""
BTF SKB Function Discoverer
Automatically discovers all kernel functions that handle sk_buff
Similar to pwru's approach but fully customizable
"""
...
class SKBFunction:
    ...
    name: str
    type: str  # kprobe, fentry, kretprobe
    category: str  # network, nft, netfilter, tcp, ip, etc.
...
class BTFSKBDiscoverer:
    """Discovers kernel functions handling sk_buff using BTF"""
    
    # Categories for function classification
    CATEGORIES = {
        'nft': ['nft_', 'nf_tables_'],
    ...
    # High priority functions (always trace)
    PRIORITY_FUNCTIONS = {
        0: [  # Critical - core packet path
            'nft_do_chain',
            'nf_hook_slow',
    ...
    def discover_from_btf(self) -> List[SKBFunction]:
        """Discover SKB functions using BTF"""
        print("[*] Discovering SKB functions from BTF...")
        
        # Method 1: Parse bpftool btf dump
        self._discover_via_bpftool()
        
        # Method 2: Parse kallsyms (fallback)
    ...
    def _discover_via_bpftool(self):
        """Discover using bpftool btf dump"""
        try:
            # Dump kernel BTF
            ...
            # Parse output for sk_buff functions
    ...
    def filter_by_priority(self, max_priority: int) -> List[SKBFunction]:
        """Filter functions by priority (0=highest)"""
